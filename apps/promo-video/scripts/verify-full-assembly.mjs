...
if (videoInfo.size < 3_000_000) throw new Error(`整片预览文件过小：${videoInfo.size}`);
const video = inspectMp4(await readFile(videoPath));
const expectedWidth = isTwoK ? 2560 : 1280;
const expectedHeight = isTwoK ? 1440 : 720;
if (video.width !== expectedWidth || video.height !== expectedHeight) {
  throw new Error(`整片尺寸错误：${video.width}x${video.height}`);
}
if (Math.abs(video.durationSeconds - 44.6) > 0.05) {
  throw new Error(`整片预览时长错误：${video.durationSeconds}`);
}
if (video.samples !== 1338) throw new Error(`整片预览帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`整片预览编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("整片预览应包含剪辑配乐与保留的 UI 音效");
...
console.log(JSON.stringify({
  ...
  assembly: {
    ...
    totalFrames: 1338,
    hardCutsAdded: 0,
    genericCrossfadesAdded: 0,
    unifiedBackdrop: "#0B1411",
    pictureLockedMusicEdit: "brand-resolution-minimax-cut-v01.wav",
    sourceMusicSegmentsSeconds: [
      [17.382, 36.857],
      [62.621, 78.471],
      [135.75, 145.725],
    ],
    musicCrossfadeSeconds: 0.35,
    musicCrossfadeCentersSeconds: [19.3, 34.8],
    finalClimaxHitSeconds: 37.8,
    retainedUiSoundEffects: true,
    finalMusicFadeSeconds: 1.6,
  },
  note: isTwoK
    ? "Native 2560x1440 2K delivery render"
    : "1280x720 review render; source composition remains native 2560x1440",
}, null, 2));
