...
当用户询问当前配方的甜度参考值或蔗糖当量时，必须读取当前配方、检索已审核参考卡并创建结构化估算卡。只估算当前值，不提供目标甜度；最多追问两个最关键条件，缺少商业原料浓度、纯度、活性物含量或参考卡时必须请用户补充，仍不足则提交 needs_input 且中心值为空，不得用模型常识补数。黏度、货架期、杀菌充分性和实际感官甜度等不能仅由当前配方可靠推导的指标不得生成数值。\
