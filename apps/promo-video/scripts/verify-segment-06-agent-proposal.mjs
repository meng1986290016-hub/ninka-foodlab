...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-segment-06-agent-proposal-v01.mp4",
);
...
if (videoInfo.size < 500_000) throw new Error(`第六段样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 7.1) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 213) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第六段样片应包含原创临时音轨");
...
console.log(JSON.stringify({
  ...
  motion: {
    ...
    nativeProcessingSteps: 3,
    nativeProposalIngredientRows: 6,
    nativeOutputPreserved: true,
    customResultCardShown: false,
    inputTextAlignedToNativeComposer: true,
    singleAlignedProcessingFocus: true,
    finalTableCentered: true,
    formalWriteActionShown: false,
  },
