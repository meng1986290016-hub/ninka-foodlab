...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-current-assembly-through-agent-v01.mp4",
);
...
if (videoInfo.size < 1_000_000) throw new Error(`当前整合样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 27.7) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 831) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("当前整合样片应包含统一原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  assembly: {
    approvedOpeningThroughIngredientsFrames: 351,
    approvedWorkbenchRecalculationFrames: 228,
    approvedAgentCapabilitiesFrames: 252,
    hardCutsAdded: 0,
  },
  audioSource: "single looping original synthetic temporary rhythm bed plus retained SFX",
}, null, 2));
