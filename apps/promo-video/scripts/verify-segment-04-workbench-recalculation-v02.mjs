...
if (videoInfo.size < 300_000) throw new Error(`第四段 V02 样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 7.6) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 228) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第四段 V02 样片应包含原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  revision: {
    nativeIngredientRowMatchMoveFrames: 30,
    fullWorkbenchHoldFrames: 28,
    continuousCameraPushStartsAtFrame: 58,
  },
