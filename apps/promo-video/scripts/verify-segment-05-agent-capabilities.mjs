...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-segment-05-agent-capabilities-v01.mp4",
);
...
if (videoInfo.size < 300_000) throw new Error(`第五段样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 8.4) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 252) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第五段样片应包含原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  motion: {
    workbenchPullbackFrames: 30,
    nativeAgentEntryClickFrame: 35,
    questionTypingFrames: 32,
    capabilityRows: 4,
    finalInputRefocus: true,
  },
