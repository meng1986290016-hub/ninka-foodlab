...
- [x] **Step 1:** 用 Image Gen 生成完整工作台和关键状态概念。
- [x] **Step 2:** 用 `view_image` 检查文字、密度和组件细节。
- [x] **Step 3:** 提取设计 token、组件和响应式规则。
- [x] **Step 4:** 提交已确认的设计规格。
...
- [x] **Step 1:** 写完整添加/编辑/删除流程测试。
- [x] **Step 2:** 按设计规格实现桌面工作台。
- [x] **Step 3:** 实现窄屏布局与键盘可达性。
- [x] **Step 4:** 使用 Browser/IAB 和截图逐项比对设计稿。
- [x] **Step 5:** 提交 `feat(recipes): add recipe workbench`。
...
- [x] **Step 1:** 写人工保存、快照不变和循环引用测试。
- [x] **Step 2:** 实现保存前预览与确认。
- [x] **Step 3:** 实现半成品版本选择与升级操作。
- [x] **Step 4:** 验证原料价格更新不改变历史版本。
- [x] **Step 5:** 提交 `feat(recipes): save immutable recipe versions`。
...
- [x] **Step 1:** 写搜索、详情、复制、归档和引用保护测试。
- [x] **Step 2:** 实现配方库与版本详情。
- [x] **Step 3:** 实现按当前价格临时重算，不覆盖快照。
- [x] **Step 4:** 提交 `feat(recipes): add recipe library`。
