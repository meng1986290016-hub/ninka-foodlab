...
const videoPath = path.join(packageRoot, "out/ninka-foodlab-combined-first-three-v01.mp4");
...
if (videoInfo.size < 300_000) throw new Error(`合并样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 11.7) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 351) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("合并样片应包含统一原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  assembly: {
    lockedSegment01Frames: 105,
    lockedSegment02Frames: 132,
    segment02To03OverlapFrames: 12,
    approvedSegment03TrimBeforeFrames: 24,
  },
  audioSource: "single original synthetic temporary rhythm bed plus retained SFX",
}, null, 2));
