...
const videoPath = path.join(packageRoot, "out/ninka-foodlab-launch-animatic-v01.mp4");
const keyframeDirectory = path.join(packageRoot, "out/review-launch-animatic-v01");
...
if (videoInfo.size < 300_000) throw new Error(`Animatic 文件过小：${videoInfo.size}`);
const video = inspectMp4(await readFile(videoPath));
if (video.width !== 1280 || video.height !== 720) {
  throw new Error(`Animatic 尺寸错误：${video.width}x${video.height}`);
}
if (Math.abs(video.durationSeconds - 40.4) > 0.05) {
  throw new Error(`Animatic 时长错误：${video.durationSeconds}`);
}
if (video.samples !== 1212) throw new Error(`Animatic 帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`Animatic 编码错误：${video.codec}`);
if (video.hasAudio) throw new Error("无声 Animatic 不应包含音轨");

const keyframes = (await readdir(keyframeDirectory))
  .filter((file) => file.startsWith("launch-animatic-keyframe-") && file.endsWith(".png"))
  .sort();
if (keyframes.length !== 23) {
  throw new Error(`Animatic 关键帧数量应为 23，实际为 ${keyframes.length}`);
}
