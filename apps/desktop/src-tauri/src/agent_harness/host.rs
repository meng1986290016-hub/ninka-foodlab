...
const WEB_PRESET_NAME: &str = "foodlab-web";
...
impl HarnessHost {
    ...
    fn prepare_profile(&self, launch: &HarnessLaunchEnvironment) -> Result<(), String> {
        ...
        let web_preset = self.home.join(".agent-presets").join(WEB_PRESET_NAME);
        create_private_directory(&preset)?;
        create_private_directory(&web_preset)?;
        ...
        write_private_file(
            ...
            "name: FoodLab\ndescription: FoodLab 本地受限食品研发 Agent，仅提供声明的本地工具。\n"
                .as_bytes(),
        )?;
        write_private_file(
            &preset.join("agent.cordis.yml"),
            agent_preset(mcp_enabled, false).as_bytes(),
        )?;
        write_private_file(
            &web_preset.join("preset.yml"),
            "name: FoodLab Web\ndescription: 仅供明确需要联网查证的 FoodLab 合规任务。\n"
                .as_bytes(),
        )?;
        write_private_file(
            &web_preset.join("agent.cordis.yml"),
            agent_preset(mcp_enabled, true).as_bytes(),
        )?;
        Ok(())
    }
}
...
fn agent_preset(mcp_enabled: bool, include_web: bool) -> String {
...
fn response_error(value: &Value) -> String {
    ...
    match code {
        ...
        "attachment-error" | "ATTACHMENT_ERROR" => attachment_error_message(&diagnostic),
...
fn attachment_error_message(diagnostic: &str) -> String {
    ...
    {
        return "当前模型仅支持文本，不能读取图片。请切换到标注为“支持图片”的模型后重试".into();
    }
    if diagnostic.contains("too_large")
        || diagnostic.contains("file size")
        || diagnostic.contains("max_bytes")
    {
        return "图片文件超过当前模型允许的大小，请压缩后重试".into();
    }
    if diagnostic.contains("too_many") || diagnostic.contains("image count") {
        return "一次添加的图片数量超出限制，请分批处理".into();
    }
    if diagnostic.contains("dimension")
        || diagnostic.contains("pixels")
        || diagnostic.contains("resolution")
    {
        return "图片尺寸或总像素超出限制，请缩小图片后重试".into();
    }
    if diagnostic.contains("format")
        || diagnostic.contains("media_type")
        || diagnostic.contains("mime")
    {
        return "图片格式与文件内容不匹配，请换用 JPG、PNG 或 WebP".into();
    }
    if diagnostic.contains("corrupt")
        || diagnostic.contains("decode")
        || diagnostic.contains("read")
    {
        return "图片损坏或无法读取，请重新导出或上传".into();
    }
    "附件未能完成校验或发送，请检查文件后重试".into()
}
...
mod tests {
    ...
    fn generated_profile_disables_high_risk_capabilities() {
        ...
        let preset = agent_preset(true, false);
        let web_preset = agent_preset(true, true);
        ...
        assert!(!preset.contains("tool-web"));
        assert!(web_preset.contains("tool-web"));
        assert!(web_preset.contains("fetch: false"));
    ...
    #[test]
    fn attachment_failures_keep_actionable_safe_reasons() {
        let cases = [
            ("model_does_not_support_images", "仅支持文本"),
            ("file_too_large", "超过当前模型允许的大小"),
            ("too_many_images", "图片数量超出限制"),
            ("pixel_dimensions_exceeded", "尺寸或总像素超出限制"),
            ("unsupported_mime_format", "JPG、PNG 或 WebP"),
            ("corrupt_image_decode_failed", "图片损坏或无法读取"),
        ];
        for (reason, expected) in cases {
            let message = response_error(&json!({
                "error": {
                    "code": "attachment-error",
                    "details": { "reason": reason, "privatePath": "/secret/file.jpg" }
                }
            }));
        ...
        assert_eq!(
            response_error(&json!({ "error": { "code": "MISSING_CREDENTIAL" } })),
            "当前模型尚未配置密钥"
        );
    }
