...
const videoPath = path.join(packageRoot, "out/ninka-foodlab-segment-02-product-reveal-v01.mp4");
...
if (Math.abs(video.durationSeconds - 4.4) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 132) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第二段样片应包含原创临时音轨");
