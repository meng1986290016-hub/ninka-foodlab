...
- 应用版本：`0.1.0`。
- 来源基线：`e5c8917`（本地 `main`）及尚未提交的 Ninka FoodLab 图标、命名与 Codex CLI 兼容性改动。
...
| `.app` 打包 | 通过 | Tauri 生成 `Ninka FoodLab.app`；`CFBundleDisplayName` 与 `CFBundleName` 均为 `Ninka FoodLab` |
...
| DMG 内容 | 通过 | 只读挂载后卷名为 `Ninka FoodLab`，包含 `Ninka FoodLab.app` 和指向 `/Applications` 的快捷入口 |
...
| 真实用户数据隔离 | 通过 | 冒烟测试只使用 `/private/tmp/food-rd-release-smoke.*` 临时 HOME，未读取或修改正式数据目录 |
