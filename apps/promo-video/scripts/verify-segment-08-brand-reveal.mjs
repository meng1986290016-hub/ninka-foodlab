...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-segment-08-brand-reveal-v01.mp4",
);
...
if (videoInfo.size < 250_000) throw new Error(`第八段样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 3.2) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 96) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第八段样片应包含原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  motion: {
    exactSegment07SymbolStart: true,
    officialTransparentSymbol: true,
    officialTransparentWordmark: true,
    bitmapBackgroundPlateShown: false,
    featureSummaryShown: false,
    horizontalBrandLockupCentered: true,
    readyForSegment09: true,
  },
