...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-reference-language-style-test-01.mp4",
);
...
if (videoInfo.size < 100_000) throw new Error(`样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 12.5) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 375) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("风格样片应包含原创临时音轨");

const keyframes = (await readdir(keyframeDirectory))
  .filter((file) => file.startsWith("reference-style-test-keyframe-") && file.endsWith(".png"))
  .sort();
if (keyframes.length !== 16) {
  throw new Error(`风格样片关键帧数量应为 16，实际为 ${keyframes.length}`);
}
...
console.log(JSON.stringify({
  ...
  audioSource: "original synthetic temporary soundtrack",
}, null, 2));
