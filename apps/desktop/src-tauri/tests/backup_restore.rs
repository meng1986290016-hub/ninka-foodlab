...
fn preflight_and_restore_replace_real_files_after_creating_a_safety_backup() {
    ...
    assert_eq!(preflight.source_schema_version, 13);
    assert_eq!(preflight.target_schema_version, 13);
    ...
    assert_eq!(result.restored_schema_version, 13);
    assert!(
        result
    ...
    assert_eq!(inspect_offline_backup(&safety).unwrap().schema_version, 13);
