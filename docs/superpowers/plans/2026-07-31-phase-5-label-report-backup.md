...
- [ ] **Step 1:** 写实施日前、实施日和实施日后的金样测试。
- [ ] **Step 2:** 实现规则包注册表与推荐函数。
- [ ] **Step 3:** 验证旧规则包始终可按 ID 读取。
- [ ] **Step 4:** 提交 `feat(labels): add GB 28050 dual rule packs`。
...
- [ ] **Step 1:** 写 2011、2025、未知、零值、边界值和单位错误测试。
- [ ] **Step 2:** 实现确定性标签计算。
- [ ] **Step 3:** 写重复运行字节级一致性测试。
- [ ] **Step 4:** 提交 `feat(labels): calculate China nutrition labels`。
...
- [ ] **Step 1:** 建立首批官方示例与自有边界夹具。
- [ ] **Step 2:** 建立规则台账。
- [ ] **Step 3:** 验证双标准差异。
- [ ] **Step 4:** 提交 `test(labels): add GB 28050 golden fixtures`。
...
- [ ] **Step 1:** 先完成信息架构和高保真设计。
- [ ] **Step 2:** 写导航、规则选择、来源复核和发布保护测试。
- [ ] **Step 3:** 实现工作台和配方库入口。
- [ ] **Step 4:** 完成真实浏览器三尺寸验收。
- [ ] **Step 5:** 提交 `feat(labels): add nutrition label workspace`。
