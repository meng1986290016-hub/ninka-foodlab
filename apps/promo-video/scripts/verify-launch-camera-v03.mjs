...
const videoPath = path.join(packageRoot, "out/ninka-foodlab-launch-camera-v03-preview.mp4");
const keyframeDirectory = path.join(packageRoot, "out/review-launch-camera-v03");
...
if (videoInfo.size < 300_000) throw new Error(`预览文件过小：${videoInfo.size}`);
const video = inspectMp4(await readFile(videoPath));
if (video.width !== 1280 || video.height !== 720) {
  throw new Error(`预览尺寸错误：${video.width}x${video.height}`);
}
if (Math.abs(video.durationSeconds - 40.4) > 0.05) {
  throw new Error(`预览时长错误：${video.durationSeconds}`);
}
if (video.samples !== 1212) throw new Error(`预览帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`预览编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("V03 预览应包含临时音乐与音效轨道");

const keyframes = (await readdir(keyframeDirectory))
  .filter((file) => file.startsWith("launch-camera-v03-keyframe-") && file.endsWith(".png"))
  .sort();
if (keyframes.length !== 32) {
  throw new Error(`V03 关键帧数量应为 32，实际为 ${keyframes.length}`);
}
