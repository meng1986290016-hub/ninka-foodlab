...
Ninka FoodLab 是一个面向食品研发人员的开源本地应用。当前已经完成供应商维度的原料库、配方工作台、配方库、原料资料导入和食品研发 Agent：同一种原料可以保存多家供应商版本，配方可以引用明确的供应商原料或正式半成品版本，并实时估算营养、成本和目标达成情况。
...
当前配置可以生成 macOS DMG/App 与 Windows NSIS 安装包。项目不使用付费开发者证书：Windows 产物不做 Authenticode 签名；macOS 只使用无需 Apple 账号的 ad-hoc 技术签名，以满足 Apple Silicon 对可执行应用的基本要求，但不包含 Developer ID 签名或公证。因此两个系统首次安装时都可能显示安全提示。

本机打包命令、GitHub 手动构建方式、产物校验和未签名安装提示见[桌面发布与安装说明](../../docs/desktop-release.md)。工作流只上传构建产物，不会自动创建 GitHub Release 或对外发布。
...
3. 录入用量和单位。可锁定固定用量，再把另一项设为“补足”，使其随其他用量变化自动补至计划投料总量。
4. 需要换算 `mL/L` 时，原料必须填写密度。缺少价格、密度或营养时，界面会保留可编辑草稿并指出缺失来源。
5. 录入出成重量、包装和附加成本，以及唯一的研发备注框。出成重量会用于折算得率、每 100g 营养和单位成本。
...
仓库提供人工触发的 macOS 与 Windows 安装包构建工作流。工作流会执行类型检查、品牌资源校验和对应平台的 Tauri 发布构建；完整的前端与 Rust 回归仍应在发布前按上面的命令单独执行。Tauri capability 当前启用 `core:default` 以及备份/导出所需的原生打开、保存对话框权限，没有 shell 权限或不受限制的文件系统权限。
