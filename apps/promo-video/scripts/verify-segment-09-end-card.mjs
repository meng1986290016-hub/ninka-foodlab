...
const videoPath = path.join(
  packageRoot,
  "out/ninka-foodlab-segment-09-end-card-v01.mp4",
);
...
if (videoInfo.size < 250_000) throw new Error(`第九段样片文件过小：${videoInfo.size}`);
...
if (Math.abs(video.durationSeconds - 3.6) > 0.05) {
  throw new Error(`样片时长错误：${video.durationSeconds}`);
}
if (video.samples !== 108) throw new Error(`样片帧数错误：${video.samples}`);
if (video.codec !== "h264") throw new Error(`样片编码错误：${video.codec}`);
if (!video.hasAudio) throw new Error("第九段样片应包含原创临时音轨");
...
console.log(JSON.stringify({
  ...
  video,
  content: {
    exactSegment08LockupStart: true,
    officialTransparentLockup: true,
    taglineShown: "食品研发的本地工作台",
    repositoryPathShown: "github.com/meng1986290016-hub/ninka-foodlab",
    featureSummaryShown: false,
    bitmapBackgroundPlateShown: false,
    finalCardSafeAreaChecked: true,
  },
